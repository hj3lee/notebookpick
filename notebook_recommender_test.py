...
df["weight_score"] = (
    -0.2 * df["weight_diff_pct"] * standard_mult[user_input["weight"] - 1]*2
).clip(-10, 20).round(2)

df["battery_score"] = (
    df["battery_time_centered"] * standard_mult[user_input["battery"] - 1]*1.5
).clip(-10, 10).round(2)
...
result_df = df.sort_values("overall_score", ascending=False).head(30)
